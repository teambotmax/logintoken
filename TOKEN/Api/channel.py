...
class Channel:
    ...
    def __init__(self, authToken,obs):
    ...
    def login(self):
        ...
        print ("obs_token:" + self.obs_token)
    ...
    def getNote(self,gid, commentLimit, likeLimit):
        ...
        r = requests.get(
            "http://" + self.host + "/mh/api/v27/post/list.json?homeId=" + gid + "&commentLimit=" + str(commentLimit) + "&sourceType=TALKROOM&likeLimit=" + str(likeLimit),
            headers = header
        )
        return r.json()
    
    def getGroupPicture(self,gid):
        res = self.getNote(gid,0,0)
        print(res)
        return res["result"]["homeInfo"]["groupHome"]["pictureUrl"]

    def uploadGroupPicture(self, path, gid):
        request_url = "https://obs-de.line-apps.com/os/g/%s" % gid
